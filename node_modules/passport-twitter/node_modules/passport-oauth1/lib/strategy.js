...
OAuthStrategy.prototype.authenticate = function(req, options) {
    options = options || {};
    if (!req.session) {
        return this.error(new Error('OAuthStrategy requires session support. Did you forget app.use(express.session(...))?'));
    }
    ...
    if (req.query && req.query.oauth_token) {
        ...
        if (!req.session[self._key]) {
            return self.error(new Error('Failed to find request token in session'));
        }
